...
import {
    ...
    CircularProgress, Button, TextField, useTheme, useMediaQuery
} from '@mui/material';
...
function MobileStockContent({
    ...
    // Add this state for managing the input field
    const [inputValue, setInputValue] = useState(quantity.toString());

    // Update inputValue when quantity changes from outside
    React.useEffect(() => {
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            ...
            <Box sx={{
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                    ...
                    {/* Input field for typing quantity */}
                    <TextField
                        type="number"
                        value={inputValue} // You'll need to add this state
                        onChange={(e) => {
                            // Allow any value during typing, including empty string
                            setInputValue(e.target.value);
                        }}
                        onBlur={() => {
                            // When focus leaves the field, enforce validation
                            ...
                            // Enforce limits
                            const validatedValue = Math.min(Math.max(1, newValue), maxQuantity);
                            setQuantity(validatedValue);
                            setInputValue(validatedValue.toString());
                        }}
                        onKeyDown={(e) => {
                            // Update quantity on Enter key
                            if (e.key === 'Enter') {
                                const newValue = parseInt(inputValue) || 1;
                                const maxQuantity = tradeType === 'buy'
                                    ? calculateMaxBuyQuantity(stockDetails, portfolio)
                        ...
                        InputProps={{
                            ...
                            // Remove spinner arrows with CSS
                            sx: {
                ...
                <Button
                    ...
                    disabled={
                        ...
                        (tradeType === 'sell' && (
                            quantity === 0 || !selectedStock || selectedStock.quantity < quantity
                        ))
                    }
                >
